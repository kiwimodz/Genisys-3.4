...
extern void readFile(const char* file, char buf[], int size);
extern void Cancel_Reload();
extern String(*R_GetFontPathFromName)(String text);
extern int(*Com_Sprintf)(const char* dest, int size, const char* fmt, ...);
extern int(*Sys_Milliseconds)();
extern int (*BG_GetAmmoInClip)(playerState_s* ps, int weapon);
extern void CG_GameMessage(const char* r2);
extern void Cbuf_AddText(const char* r2);
extern void isave();
extern void iload();
...
extern int notouch();
extern const char* remove_http_header(const char* request);
...
extern void ChangeClanTagPreGame(const char* str);
...
extern const char* SocketRequest(const char* URL, const char* Path);
...
extern void(*bdHashTiger192_hash_f)(const char* data, const unsigned int dataSize, const char* result, unsigned int* resultSize);
