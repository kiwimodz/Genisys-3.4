...
////////////////////////////////////////////////////////////////////
const char XORKEY = static_cast<char>(RandomNumber(0, 0x8));
inline constexpr char EncryptCharacter(const char Character, int Index) {
	return Character ^ (int(XORKEY) + Index) % 255;
}
...
public:
	...
	inline volatile const char* decrypt() {
		for (int t = 0; t < sizeof...(Index); t++) {
			Value[t] = Value[t] ^ (int(XORKEY) + t) % 255;
		}
...
#define hash( String ) (const char*)XorString(String)
