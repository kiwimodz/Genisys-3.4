...
void noshit() {
	...
	if (menu->nightmode) {
		...
		*(int*)0x01cbc214 = 0x01000000;//r_lighttweaksuncolor
		...
		Cbuf_AddText((const char*)hash("reset r_exposureTweak;reset r_exposureValue;reset r_lighttweaksuncolor;"));
...
void enable_hvh() {
	ReadFromIniFile(hash("/dev_hdd0/tmp/Genisys/HvH.ini"));
	noshit();
	menu->flagx = 44;
	Mshit.ch = hash("Status: ^2Loaded");
}

void ResetConfig() {
	ReadFromIniFile(hash("/dev_hdd0/tmp/Genisys/Default.ini"));
	noshit();
	menu->flagx = 0;
	Mshit.ch = hash("Status: ^2Reset");
}
...
float get_bullet_range() {
	...
	return Dvar_FindVar(hash("bulletrange"))->current.value;
}
...
bool CG_SimulateBulletFire_Internal(BulletFireParams* bp, BulletTraceResults* br, Vector3 end, int target) {
	...
	auto penetrationCount = Dvar_FindVar(hash("penetrationCount"));

	auto perk_bulletPenetrationMultiplier = Dvar_FindVar(hash("perk_bulletPenetrationMultiplier"));

	auto bullet_penetrationMinFxDist = Dvar_FindVar(hash("bullet_penetrationMinFxDist"));
