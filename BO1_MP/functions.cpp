	...
	if (menu->flshname)
	{
		...
		if (istate)
		{
			...
			else
			{
				...
				Com_Sprintf(Addver, sizeof(Addver), "^%i%s", irand_(0, 8), str);
...
void local_command_config(int localClientNum, const wchar_t* wstr, unsigned int length) {
	...
	sprintf(FileName, "%s%s.ini", FilePath, buffer);
	sprintf(File, "%s.ini", buffer);
...
void DelConfig()
{
	...
	ProcessConfig();
	Mshit.ch = "Status: ^2Config Deleted";
...
void iload() {
	...
	Mshit.ch = "Status: ^2Config Loaded";
	noshit();
}
...
void InjectCustomGSC() {
	const char* filePath = "";
	int fileSize = getFileSize((char*)filePath);

	if (fileSize > 0) {
		memset((void*)0x10040000, 0, fileSize + 1);
		readFile((char*)filePath, (char*)0x10040000, fileSize);
...
void PartyTime() {
	if (menu->bForceInv) {
		...
		*(char*)0x535D67 = 0x90;
		*(char*)0x5359FC = 0x90;
		strcpy((char*)0x96EC34, "Game Invite");
		strcpy((char*)0x096ec4c, "MENU_GAME_INVITE_MESSAGE");
	}
...
void(*bdHashTiger192_hash_f)(const char* data, const unsigned int dataSize, char* result, unsigned int* resultSize) = (void(*)(const char*, const unsigned int, char*, unsigned int*))bdHashTiger192_hash_t;
