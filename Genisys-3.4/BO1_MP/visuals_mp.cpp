...
void HealthBar(cg_s* cgss) {
	...
	char boolBuff[100];
	Com_Sprintf(boolBuff, sizeof(boolBuff), hash("Health %i/100"), cgss->health);
...
void prox(int i) {
	if (Prox_in_Circle(centity[cg->clientNum].Origin, 350.0, centity[i].Origin)) {
		DrawTextWithEffectsUI(hash("[Enemy] is near"), cg->refdef.Width / 2, 34, .95, color(255, 0, 0, 255), align_center);
	}
}
		...
		if (centity[i].Type > 0) {
			if (WorldPosToScreenPos(EntOrigin, &ET_ItemPosition)) {
				if (centity[i].Type == ET_ITEM) {
					if (((centity[i].State & (1 << 6)) != 0)) {
						if (!strcmp(weapondefid->name, hash("scavenger_item_mp")) && bot.esp.bpickupscavsworld)//scavs
						{
			...
			if (((centity[i].State & (1 << 6)) != 0)) {
				if (bot.esp.bradrhelis) {
					...
					if (centity[i].Type == ET_HELICOPTER) {
						...
						if (!strstr(*(const char**)mat, hash("None")) && mat != NULL) {
				...
				if (bot.esp.bradrpickupscavs || bot.esp.bradrnades || bot.esp.bradrpickupweaps) {
					...
					if (bot.esp.bradrpickupscavs) {
						...
						if (!strcmp(weapondefid->name, hash("scavenger_item_mp"))) {
...
void draw_sprint_meter() {
	...
	DrawShaderEsp1(15, (((cg->refdef.Height / 2)) * 1.5) - (200 * ((Dvar_FindVar(hash("player_sprintMinTime"))->current.value * 1000) / sprint_time_max)), 8, 2, 0, color(255, 0, 0, 100), white, align_left);
}
...
void DrawNadeFuseTimer(int i) {
	...
	if (WorldPosToScreenPos(EntOrigin, &ET_ItemPosition)) {
		if (centity[i].Type == ET_MISSILE) {
			if (((centity[i].State & (1 << 6)) != 0)) {
				if ((weapondefid->weapDef->hudIcon) && (!strstr(weapondefid->weapDef->szOverlayName, hash("claymore"))) && (!strstr(weapondefid->weapDef->szOverlayName, hash("bouncing")) && (!strstr(weapondefid->weapDef->szOverlayName, hash("shock"))) && (!strstr(weapondefid->weapDef->szOverlayName, hash("tactical")) && (!strstr(weapondefid->weapDef->szOverlayName, hash("axe"))) && (!strstr(weapondefid->weapDef->szOverlayName, hash("trophy")))))) {
...
color BG_GetColor(team_t team) {
	const char* str = BG_GetFactionFromTeam_f(team, Dvar_GetString(hash("party_mapname")));
	color temp;
