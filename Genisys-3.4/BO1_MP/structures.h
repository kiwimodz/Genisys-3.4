...
union unknownXModelStruct1Internal2 {
	int verts0;
	const char* unknownOption2;			//Size = ((parent struct::vertCount << 1) + parent struct::vertCount) << 3
};
...
struct WeaponDef {
	...
	const char* loopFireEndSound;
	const char* loopFireEndSoundPlayer;
	const char* startFireSound;
	const char* stopFireSound;
	const char* killcamStartFireSound;
	const char* startFireSoundPlayer;
	const char* stopFireSoundPlayer;
	const char* killcamStartFireSoundPlayer;
	...
	const char** bounceSounds;    //Refer to surfaceNames_t
...
struct XModel {
	...
	union {
		const char* parentList;  //size = numBones - numRootBones
		const char* localParentList;  //size = numBones - numRootBones
	};
	union {
	...
	const char* partClassification;		//size = numBones
...
public:
	SessionData* session;
	SessionData* prescence;
	char pad[0x200];
	PartyMember members[18];
...
//
//////////////////////////////////////////////////////////////////////
//const char XORKEY = static_cast<char>(RandomNumber(1, 3));
//inline constexpr char EncryptCharacter(const char Character, int Index) {
//	return Character ^ (int(XORKEY) + Index) % 250;
//}
//
//
...
//public:
//	inline constexpr CXorString(const char* const String)
//		: Value{ EncryptCharacter(String[Index], Index)... } {}
//
//	inline volatile const char* decrypt() {
//		for (int t = 0; t < sizeof...(Index); t++) {
//			Value[t] = Value[t] ^ (int(XORKEY) + t) % 250;
//		}
...
//#define xor( String ) XorString(String)
#define hash( String ) String //(const char*)XorString(String)
static String keytypesDec4[] = { hash("Engages aimbot / autoshoot via L1") };
#pragma endregion
#endif