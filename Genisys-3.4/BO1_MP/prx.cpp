...
Detour Party_UpdateLocalClientDetour;
...
void Party_UpdateLocalClient(PartyData_s* party, ControllerIndex_t localControllerIndex, int memberIndex, PartyMember* member)
{
	Party_UpdateLocalClientDetour.Stub(party, localControllerIndex, memberIndex, member);
	*(int*)&member->platformAddr.liveaddr.xnaddr.externalIP = rand_int31_TT800();
	//*(int*)&member->platformAddr.liveaddr.xnaddr.interalIP = rand_int31_TT800();
	//*(short*)&member->platformAddr.liveaddr.xnaddr.port = rand_int31_TT800();
}
...
void R_AddDObjToScene(DObj* obj, int pose, unsigned int entnum, unsigned int renderFxFlags, Vector3* lightingOrigin, const float* materialTime, const float* burnFraction, int altXModel, int textureOverrideIndex, ShaderConstantSet* dobjConstantSet, float lightingOriginToleranceSq, float scale, bool isMarkableViewmodel, int rs, int rt, int rst) {
	...
	if (!runonce) {
		WeaponChams = Material_RegisterHandle(hash("mc/mtl_t5_weapon_camera_spike"), 7);
		PlayerZchams = Material_RegisterHandle(hash("mc/mtl_german_shepherd_fur_black"), 7);
		MMS_display = Material_RegisterHandle(hash("mc/mtl_t6_attach_optic_mms_display"), 6);
		rmr_lens = Material_RegisterHandle(hash("mc/mtl_t6_attach_optic_rmr_lens"), 6);
		shell_alpha = Material_RegisterHandle(hash("mc/mtl_fx_shell_alpha"), 6);
		camo_dragon_alt = Material_RegisterHandle(hash("mc/mtl_weapon_camo_dragon_alt"), 6);
		rangefinder_reticle2 = Material_RegisterHandle(hash("mc/mtl_t6_attach_optic_rangefinder_reticle2"), 6);
		optic_m32_lens = Material_RegisterHandle(hash("mc/mtl_t6_attach_optic_m32_lens"), 6);
		shotgun_shell_alpha = Material_RegisterHandle(hash("mc/mtl_fx_shotgun_shell_alpha"), 6);
		ripple_trail = Material_RegisterHandle(hash("gfx_fxt_fx_distortion_ripple_trail"), 6);
		wpn_pda_screen = Material_RegisterHandle(hash("mc/mtl_t6_wpn_pda_screen"), 6);
		weapon_camo_gold_2 = Material_RegisterHandle(hash("mc/mtl_weapon_camo_gold_2"), 6);
		...
		for (int i = 0; i < count; i++) {
			if (headers[i].material) {
				if (strstr(headers[i].material->info.name, hash("toxic")) || strstr(headers[i].material->info.name, hash("dlc4")))
	...
	if (dobjConstantSet) {
		...
		if (centity[entnum].ClientNumber == cg->clientNum) {
			if (menu->renderchamsonme) {
				R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingR")), menu->camo.r * bot.chamsintensity);//Set chameleon skin Red
				R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingG")), menu->camo.g * bot.chamsintensity);//Set chameleon skin Green
				R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingB")), menu->camo.b * bot.chamsintensity);//Set chameleon skin Blue
				...
				R_UnmapShaderConstantSet(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingR")));//Unmap chameleon skin Red
				R_UnmapShaderConstantSet(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingG")));//Unmap chameleon skin Green
				R_UnmapShaderConstantSet(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingB")));//Unmap chameleon skin Blue
				R_AddDObjToSceneDetour.Stub(obj, pose, entnum, renderFxFlags, lightingOrigin, materialTime, burnFraction, altXModel, textureOverrideIndex, dobjConstantSet, lightingOriginToleranceSq, scale, 0, 0, 0, 0);
			}
		}
		...
		if (centity[entnum].eType == ET_ITEM) {
			if (bot.esp.bweaponchams) {
				R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingR")), menu->camo.r * 10.f);//Set chameleon skin Red
				R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingG")), menu->camo.g * 10.f);//Set chameleon skin Green
				R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingB")), menu->camo.b * 10.f);//Set chameleon skin Blue
			}
		...
		if (centity[entnum].Type == ET_PLAYER || (centity[entnum].Type == ET_DOG)) {
			if (bot.esp.bchams) {
				if (CG_IsEntityFriendlyNotEnemy(&centity[entnum]) && bot.esp.ballies) {
					R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingR")), menu->esp[entnum].r * bot.chamsintensity);//Set chameleon skin Red
					R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingG")), menu->esp[entnum].g * bot.chamsintensity);//Set chameleon skin Green
					R_SetShaderConstantSetValueW(dobjConstantSet, R_AllocShaderConstantSet(dobjConstantSet, hash("heroLightingB")), menu->esp[entnum].b * bot.chamsintensity);//Set chameleon skin Blue
				}
...
#define ingame Dvar_GetBool(hash("cl_ingame"))
bool ingame_;
#pragma endregion
...
bool precacheAssets() {
	...
	white = Material_RegisterHandle(hash("white"), 0);
	gradient_fadein = Material_RegisterHandle(hash("gradient_fadein"), 7);
	menu_safearea_arrow = Material_RegisterHandle(hash("menu_safearea_arrow"), 7);
	frame_color_debug = Material_RegisterHandle(hash("frame_color_debug"), 7);
	menu_mp_killstreak_select = Material_RegisterHandle(hash("menu_mp_killstreak_select"), 7);
	compassping_player = Material_RegisterHandle(hash("compassping_player"), 7);
	hud_scavenger_pickup = Material_RegisterHandle(hash("hud_scavenger_pickup"), 7);
	hint_mantle = Material_RegisterHandle(hash("hint_mantle"), 7);
	compassping_dog = Material_RegisterHandle(hash("compassping_dog"), 7);
	compass_sg_white = Material_RegisterHandle(hash("compass_sg_white"), 7);
	compass_turret_white = Material_RegisterHandle(hash("compass_turret_white"), 7);
	compass_hk = Material_RegisterHandle(hash("compass_hk"), 7);
	waypoint_recon_artillery_strike = Material_RegisterHandle(hash("waypoint_recon_artillery_strike"), 7);
...
int R_EndFrame() {
	...
	if (!assets_precached) {
		Mshit.ch = hash("Status: Waiting");
		local->stausrank = hash("Status: Waiting");
		if (precacheAssets())
...
void Com_Error(int code, const char* fmt, const char* ap) {
	char buffer[400];
...
void LUI_LuaCall_DrawRectangleH(LUI_Element* r3, float f1, float f2, float f3, float f4, float red, float green, float blue, float alpha, int r9, int r10) {
	...
	if (menu->bo2theme && !cl_ingame_()) {
		...
		if (strstr(icon, hash("menu_mp_soldiers"))) {
			red = menu->skin.r;
			green = menu->skin.g;
			blue = menu->skin.b;
			alpha = menu->skin.a / 2;
		...
		if (strstr(icon, hash("lui_bkg"))
			|| strstr(icon, hash("menu_mp_cac_backing"))
			|| strstr(icon, hash("menu_mp_backing_frame")) || strstr(icon, hash("holotable"))) {
			red = menu->skin.r;
			green = menu->skin.g;
			blue = menu->skin.b;
			alpha = menu->skin.a;
...
const char* SE_GetString_FastFile(const char* psPackageAndStringReference) {
	...
	if (!strcmp(psPackageAndStringReference, hash("MENU_SEARCHING_FOR_PLAYER"))) {
		return hash("Searching for ^1gaybab users");
	}

	if (!strcmp(psPackageAndStringReference, hash("MPUI_HOST_MIGRATION"))) {
	}

	if (!strcmp(psPackageAndStringReference, hash("MENU_SELECT_GRENADE"))) {
		return hash("Select Ball Buster");
	}

	if (!strcmp(psPackageAndStringReference, hash("WEAPON_FRAGGRENADE"))) {
		return hash("Ball Buster");
	}

	if (!strcmp(psPackageAndStringReference, hash("SCRIPT_GRENADE_DEATH"))) {
		return hash("M8 You were killed by a Ball Buster");
	}
...
void CG_FireWeaponH(int localClientNum, centity_t* cent, int event1, int eventParm, const unsigned __int16 tagName, Weapon weapon, playerState_s* ps, bool leftGun) {
	...
	*(int*)0x0FA248 = menu->bgunsound ? 0x60000000 : 0x4BFFF2AD;

	CG_FireWeaponDetour.Stub(localClientNum, cent, event1, eventParm, tagName, weapon, ps, leftGun);
}
...
void Tracer_Spawn(int localClientNum, Vector3* pstart, Vector3* pend, centity_s* instigator, int weapon) {
	if (bot.esp.btracers && cl_ingame_()) {
		...
		weapon_tracer->material = Material_RegisterHandle(hash("gfx_tracer"), 6);
		weapon_tracer->speed = 1.0f;
		weapon_tracer->name = hash("smg");
...
char CG_BulletHitEventH(int localClientNum, int sourceEntityNum, int targetEntityNum, int weapon, Vector3* endPos, int surfacetype, Vector3* normal, Vector3* seeThruDecalNormal, int nothing, int event_, char eventParam, int hitContents, char boneIndex) {
	if (cl_ingame_()) {
		if (bot.esp.btracers && sourceEntityNum == cg->clientNum) {
			...
			if (CG_PrepFireParams(0, &centity[cg->clientNum], SL_GetString(hash("tag_flash"), 7), playerState, playerState->weapon, 32, true, &FireParams, &vTracerStart, &iShots, &flRange, &Orientation, &vOrigin, &flSpread, &iIgnoreNum)) {
				if (local->alive)
...
int sceNpBasicSendMessage_f(const SceNpId* to, const void* data, size_t size) {
	...
	/*if (size == 0x58)
	{
		if (join_message_data->inviteInfo.fromMPInvite == false)
		{
			if (join_message_data->mType == JOIN_REQUEST || (join_message_data->mType == JOIN_REPLY))
			{
				char buf[256];
					Com_Sprintf(buf, sizeof(buf), encryptDecrypt(hash("Rdoe!Un![nlchdr!gsnl!$r!Cmnbjde")).c_str(), to->handle.data);
					UI_OpenToastPopup(0, encryptDecrypt(hash("ldot^lq^bnous`bu^dyqhsde")).c_str(), encryptDecrypt(hash("Xd`i!O`i!L9")).c_str(), buf, 5000);
	...
	if (*size == 88) {
		if (message->mType == JOIN_REQUEST) {
			if (message->inviteInfo.fromMPInvite == false || message->inviteInfo.fromMPInvite > 1) {
				char buf[256];
				Com_Sprintf(buf, sizeof(buf), encryptDecrypt(hash("Rdoe!Un![nlchdr!gsnl!$r!Cmnbjde")).c_str(), from->userId.handle.data);
				UI_OpenToastPopup(0, encryptDecrypt(hash("ldot^lq^bnous`bu^dyqhsde")).c_str(), encryptDecrypt(hash("Xd`i!O`i!L9")).c_str(), buf, 5000);
				*event = 0;
				return 0x8002a66a;
			}
...
int MSG_ReadShort_Hook(msg_t* msg) {
	...
	if (__builtin_return_address() == (void*)0x00332BDC && (ret < 0)) {
		//CG_GameMessage(hash("P2P Rme: Attempt Blocked\n"));
		return 0;
	}
...
void CG_DeployServerCommand_Hook(int localClientNum) {
	...
	case 105:
		...
		if (iTeamNum < 0 || iTeamNum > 10) {//max team exploit
			UI_OpenToastPopup(0, encryptDecrypt(hash("ite^nchu^ed`ui^rthbhed")).c_str(), VirtualXOR(hash("V8IYM^F/TTFVWASS"), 8).c_str(), encryptDecrypt(hash("SLD!Cmnbjde")).c_str(), 5000);//RME
			return;
		}
	case 55:
		return;
...
void ClientCommand(int ClientNum, int idk) {
	...
	if (strstr(SV_Cmd_Argv(3), hash("endround"))) {
		if (Session_IsHost_f(0x26C2BC8, ClientNum)) {
			...
			char buffer[100];
			Com_Sprintf(buffer, sizeof(buffer), hash("End Game ^1Denied ^3GenisysV3.4 ^7is smarter than %s "), cg->clients[ClientNum].PlayerName);
			SV_GameSendServerCommand(buffer);
			goto endfalse;
		}
	}

	if (strstr(SV_Cmd_Argv(0), hash("sl"))) {
		if (Session_IsHost_f(0x26C2BC8, ClientNum)) {
			goto endtrue;
		}
			...
			char buffer[100];
			Com_Sprintf(buffer, sizeof(buffer), hash("Server Crash ^1Denied ^3GenisysV3.4 ^7is smarter than %s "), cg->clients[ClientNum].PlayerName);
			SV_GameSendServerCommand(buffer);
			goto endfalse;
		}
	}
...
int LUI_CoD_ReadCharFromString_Hook(const char** str, uint32_t* r4, Vector4* r5, GfxColor* r6, const char** r7, Material** r8) {
	auto pos = check_invalid_material_handles(*str);
	if (pos != std::string::npos) {
		*str = hash("Blocked this nerds crash");
	}

	return LUI_CoD_ReadCharFromStringDetour.Stub(str, r4, r5, r6, r7, r8);
}

int SEH_ReadCharFromString_Hook(const char** str) {
	auto pos = check_invalid_material_handles(*str);
	if (pos != std::string::npos) {
		*str = hash("Blocked this nerds crash");
	}
...
void __cdecl DynEntCl_DestroyEvent_h(int localClientNum, uint16_t dynEntId, DynEntityDrawType drawType, Vector3* hitPos, Vector3* hitDir) {
	...
	if (is_crash_attempt) {
		char buffer[100];
		Com_Sprintf(buffer, sizeof(buffer), hash("Crash: Attempt Blocked\n%s"), Dvar_GetString(hash("party_hostname")));
		CG_GameMessage(buffer);
		return;
	}
...
void CG_Player(int localClientNum, centity_s* cent) {
	...
	if (!runone_25) {
		createThread(&id[5], ThreadedAimbot, 0, 100, 0x10000, 0, hash("\0"));
		runone_25 = true;
	}
	...
	if (cl_ingame_()) {
		...
		if (cent->Type == ET_PLAYER) {
			...
			if (bot.esp.ballies || bot.esp.baxis) {
				...
				AimTarget_GetTagPos_f(cent, AimBoneIndex[j_head], &local->playeresp[cent->ClientNumber].vtagesp);
			}
...
void bdCommonAddr_serialize_f(bdCommonAddr* ca, const char* buffer) {
...
void Live_JoinSessionInProgress_f(int local, uint64_t xuid) {
	if (local == -1) {
		if (temp_npid.opt[0] != '\0')
			...
			UI_OpenToastPopup(0, encryptDecrypt(hash("ldot^lq^bnous`bu^dyqhsde")).c_str(), encryptDecrypt(hash("Gtbj")).c_str(), encryptDecrypt(hash("Bntmeo&u")).c_str(), 5000);
...
void Party_AcceptInvite_f(int ctx_index, InviteMessage* message) {
	if (message->fromMPInvite == false || message->fromMPInvite > 1) {
		char buf[256];
		Com_Sprintf(buf, sizeof(buf), encryptDecrypt(hash("Rdoe!Un![nlchdr!gsnl!$r!Cmnbjde")).c_str(), hash("some dick"));
		UI_OpenToastPopup(0, encryptDecrypt(hash("ldot^lq^bnous`bu^dyqhsde")).c_str(), encryptDecrypt(hash("Xd`i!O`i!L9")).c_str(), buf, 5000);
	}
	else
...
void AuthListener() {
	...
	*(char*)0x521C38 = 0x38;
	...
	Ini.Name = hash("/dev_hdd0/tmp/Default.ini");
	bot.fltmlt = 0.58f;
	menu->Iammo = false;
...
int Menu_PaintAll(int localClientNum, UiContext* _dc) {
	if (cl_ingame_()) {
		...
		if (Title.find(hash("dev_hdd0/tmp/Genisys/Custom_Camos")) != std::string::npos) {
			WriteAsset(Title.c_str(), hash("~-gmtl_t6_wpn_toxic_camo_col"), 500);
			Memcp__((void*)0x122B3FC, (void*)0x122B234, 44);
			WriteAsset(Title.c_str(), hash("~~-gcamo_zmb_dlc4_alt_spc-rgb~fc3ae34a"), 500);
		}
		...
		if (v) {
			...
			if (Live_IsUserSignedIntoDemonware(0)) {
				...
				memcpy(&DefaultNpInfo, (void*)0x026c0658, sizeof(SceNpId));
...
static uint16_t sc_disable[] = { 200, 203, 1022, 6, 10, 11, 15, 20, 35, 36, 38 };
void disableCFWSysCalls() {

	for (uint8_t sc = 0; sc < 11; sc++)
	{
		system_call_3(8, 0x7777, 0x0092, sc_disable[sc]);
	}
}
