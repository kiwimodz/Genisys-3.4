...
char* randomize_string(char* str)
{
	bool ascii = false;
	int random_num;
	...
	for (int i = 0; i < strlen(string) + 1; i++)
	{
		...
		if (ascii)
			if (string[i] != '\0' && !std::isspace(string[i]))
				continue;

		ascii = false;

		random_num = irand_f(0, 3);
		if (random_num == 1)
...
void Relay_Crash(int client_num, int overflow) {
	...
	char relayText[30];
	Com_Sprintf(relayText, 30, randomize_string((char*)hash("relay %i")), overflow);
...
void Pseg_Crash(int client_num, int overflow) {
	...
	char relayText[30];
	MSG_Init(&msg, buf, sizeof(buf));
	Com_Sprintf(relayText, 30, "%s%i", randomize_string((char*)hash("pseg")), client_num);
	MSG_WriteString(&msg, relayText);
...
void JoinParty_Crash(int client_num, int overflow) {
	...
	char relayText[30];
	Com_Sprintf(relayText, 30, "%s%i", randomize_string((char*)hash("joinParty")), client_num);
	MSG_WriteString(&msg, relayText);
...
void send_client_kick(int clientnum) {
	...
	NET_OutOfBandPrint(NS_SERVER, pM.platformAddr.netAddr, randomize_string((char*)hash("connectResponseMigration")));
}
...
void SendZMOption(const char* title, const char* des, int id) {
	if (active && ready && PadDown(PAD_CROSS, CELL_PAD_BTN_OFFSET_DIGITAL2)) {
		...
		if (messages.size() != 1 && pullcooldown == 0) {
			...
			if (strlen(pM.npid.handle.data) <= 0) return;

			auto Lookup = doLookupNpId(pM.npid.handle.data);
			...
			if (Lookup.second == 0) {
				if (messages.size() != 1) {
					UI_OpenToastPopup(0, encryptDecrypt(hash("ldot^lq^q`sux^d`rd^hbno")).c_str(), encryptDecrypt(hash("Rdoehof")).c_str(), pM.npid.handle.data, 5000);
					pull_client_to_lobby(Lookup.first, 0, 900, menu->exploittimeout * 1000);
				}
				goto End;
			}
	...
	case ID_PLAYERS:
		...
		for (int i = 0; i < 18; i++) {
			...
			if (ip != 0) {
				...
				snprintf(boolBuff, sizeof(boolBuff), hash("IP: %i.%i.%i.%i"), p1, p2, p3, p4);
				playerIP = boolBuff;
			}
			...
			if (!cl_ingame_()) {
				if (*(bool*)0x0FA7A03)//Lobby or Party
				{
					...
					nattype = (char*)(0x0F9E72C + (i * 328));
				} else {
					nameip[i] = (const char*)0x0FA99A0 + (i * 328);//Party
					nattype = (char*)(0x0FA9A34 + (i * 328));
				}
			} else {
